...
Utils.prototype.isValidNumberEnteredTripVerification = function (value,message) {
    let response = {};
    if (value) {
        // value = value;
        // if (/^[0-9]*$/.test(Number(value))) {
        if (/^\d+(?:\.\d{1,2})?$/.test(Number(value))) {
            response.status = true;
            response.message = value;
        } else {
            response.status = false;
...
Utils.prototype.LiteUserAmountCheck = function (value,fieldName,vehicleType,roleNumber) {
    let response = {};
    if (value) {
        let convertedValue = parseInt(value);
        // console.log('value type---',convertedValue,typeof(convertedValue));
        // console.log('vehicleType====',vehicleType,typeof(vehicleType));
        // console.log('roleNumber====',roleNumber,typeof(roleNumber));
        if (/^[0-9]*$/.test(Number(value))) {
            // if (convertedValue > 0) {
                if (roleNumber === 1){
                    if (convertedValue <= 1000){
                        ...
                        response.status = false;
                        response.message = 'Associate shift amount cannot exceed 1000';
                    }
                }else {
                    if (vehicleType === 2 || vehicleType === 3){
                        if (convertedValue <= 1000){
                            ...
                            response.status = false;
                            response.message = vehicleType +' Wheeler amount cannot exceed 1000';
                        }
                        ...
                        if (convertedValue <= 2000){
                            ...
                            response.status = false;
                            response.message = vehicleType +' Wheeler amount cannot exceed 2000';
                        }
                    }else {
                        if (convertedValue <= 1000 || convertedValue === ''){
                            response.status = true;
                            response.message = value;
                        }else {
                            response.status = false;
                            response.message = 'Allowed amount cannot exceed 1000';
                        }
                    }
                }
            // } else {
...
//used for bank screen validations
//returning for editable,textinputs
Utils.prototype.checkBankValueEnteredToEditing = function (canEditTextInput,value) {
    let name = value;
    // console.log('name====',name , name.length,name.type);
    if (canEditTextInput){
...
//used for bank screen validations
//returning for disable touchable opacity
Utils.prototype.checkBankValuePresentToEditing = function (canEditTextInput,value) {
    if (canEditTextInput){
        if (value) {
            return true
        } else {
