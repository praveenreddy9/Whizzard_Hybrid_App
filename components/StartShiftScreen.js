                    ...
                    <Card.Title
                        ...
                        right={() =>
                            <View style={[Styles.row, {paddingRight: 10}]}>
                                ...
                                <TextInput
                                    style={[Styles.txtAlignCen,Styles.tInputH40, {width: 80, fontWeight: 'bold', fontSize: 17}]}
...
//Odometer reading validate
    ...
    ReadingsCountSTART = (body) => {
        const {UserFlow,CurrentShiftId,mocked} = this.state;
        const self = this;
      let apiURL;
        if (this.state.selectedTripValue === "NO_LOAD") {
   ...
   requestCurrentLocation = async()=>{
    ...
    setPhoneNumberSelectedValue(searchPhoneNumberOthers){
        const {StartShiftDetails} =  this.state;
        this.setState({searchPhoneNumberOthers},()=>{
    ...
    render() {
        ...
        const {checkOdometerReading,requireOdometerDetails,requirePackageDetails,StartShiftDetails,searchPhoneNumberOthers} =this.state;
        return (
                                                        ...
                                                        this.state.tripTypeList.map(item => {
                                                            return (
                                                                <List.Item
                                                                    ...
                                                                    key={item.value}
                                                                    title={item.name}
                                                                    titleStyle={[Styles.ffMregular, Styles.colorBlue, Styles.f16, Styles.aslCenter, Styles.bw1,Platform.OS === 'ios' ? Styles.br0 : Styles.br100,
