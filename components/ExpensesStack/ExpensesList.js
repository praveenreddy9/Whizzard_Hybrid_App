...
export class ExpensesList extends Component {
    ...
    onBack = () => {
        if (this.state.createReimbursementModal) {
            return this.props.navigation.navigate('ExpensesList');
        }else {
        ...
        this.setState({spinnerBool: true}, () => {
            Services.AuthHTTPRequest(apiUrl, 'GET', body, function (response) {
                if (response.status === 200) {
                    // console.log("get filter roles resp200", response.data);
                    let tempResponse = []
    ...
    getFilterSitesList() {
        ...
        this.setState({spinnerBool: true}, () => {
            Services.AuthHTTPRequest(apiUrl, 'GET', body, function (response) {
                if (response.status === 200) {
                    // console.log("filter sites list resp200", response.data);
        ...
        this.setState({spinnerBool: true}, () => {
            Services.AuthHTTPRequestForShiftFlow(apiUrl, 'POST', body, function (response) {
                if (response.status === 200) {
                    ...
                    self.getBusinessUnits()    //to get business units list to create
                }
            }, function (error) {
                // console.log('==============@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@states')
    ...
    createExpenseRequest() {
        ...
        this.setState({spinnerBool: true}, () => {
            Services.AuthHTTPRequestForShiftFlow(apiUrl, 'POST', body, function (response) {
                ...
                console.log('create Expense error======>', error, error.response, error.response.data);
                self.errorHandling(error)
            })
        })
    };
    ...
    getExpenseDetails(expenseId) {
        ...
        this.setState({spinnerBool: true}, () => {
            Services.AuthProfileHTTPRequest(apiUrl, 'GET', body, function (response) {
                if (response.status === 200) {
    ...
    //To check Paid expenses //for Finance //error for mobile
    getPaidMonthlyReport(userId) {
        ...
        this.setState({spinnerBool: true}, () => {
            Services.AuthHTTPRequestForShiftFlow(apiUrl, 'POST', body, function (response) {
                ...
                console.log('get Monthly Report error', error.response);
                self.errorHandling(error)
            })
        })
    };

    //Finance Roles //cross check others
    ...
    //PAID for self or others //check with finace role
    getPaidExpenseList() {
        const self = this;
    ...
    setDateTimePicker = (event, date) => {
        ...
        this.setState({expenseDate: shiftDate,showDateTimepicker:false});
    };
    ...
    setExpenseSelectedValue(categoryType){
    ...
    //to create,update,view the expenses details
    returnCreateExpenseDetails(){
        const {
                        ...
                        <View style={[Styles.flex1, Styles.padH10, Styles.mBtm15]}>
...

                                {
                                    this.state.showDateTimepicker
                                        ...
                                        <RNDateTimePicker
                                            ...
                                            mode={'date'}
                                            value={new Date(this.state.expenseDate)}
                                ...
                                <Text style={[Styles.ffLBlack, Styles.f18, Styles.aslStart]}>Main
                                    ...
                                    <View style={[Styles.row, Styles.aslCenter, Styles.padV3]}>
                                        <View style={[Styles.row, Styles.aslCenter]}>
                                            <RadioButton.Android value={"STATION"} disabled={!canEditTextInput}
                                                         status={this.state.categoryType === "STATION" ?'checked' : 'unchecked'}
                                                         onPress={()=>{this.setExpenseSelectedValue("STATION")}}/>
                                            <Text
                                                style={[Styles.ffLBlack, Styles.padH5, Styles.aslCenter, Styles.f16]}>Station</Text>
                                        </View>
                                        <View style={[Styles.row, Styles.aslCenter]}>
                                            <RadioButton.Android value={"CENTRAL"} disabled={!canEditTextInput}
                                                         status={this.state.categoryType === "CENTRAL" ?'checked' : 'unchecked'}
                                                         onPress={()=>{this.setExpenseSelectedValue("CENTRAL")}}/>
                                            <Text
                            ...
                            <View
                                ...
                                <Text style={[Styles.ffMbold, Styles.f18, Styles.aslStart]}>Is GST
                                    ...
                                    <View style={[Styles.row, Styles.aslCenter, Styles.padV3]}>
                                        <View style={[Styles.row, Styles.aslCenter]}>
                                            <RadioButton.Android value={true} disabled={!canEditTextInput}
                                                         status={this.state.gstBill === true?'checked' : 'unchecked'}
                                                         onPress={()=>{this.setState({gstBill:true})}}/>
                                            <Text
                                                style={[Styles.ffLBlack, Styles.padH5, Styles.aslCenter, Styles.f16]}>Yes</Text>
                                        </View>
                                        <View style={[Styles.row, Styles.aslCenter]}>
                                            <RadioButton.Android value={false} disabled={!canEditTextInput}
                                                         status={this.state.gstBill === false?'checked' : 'unchecked'}
                                                         onPress={()=>{this.setState({gstBill:false})}}/>
                                            <Text
    ...
    //reports will be for finance roles use,this fun is to pay the grouped expenses
    returnUserReportDetails(){
        const {
            expensesList, status, userRole, selectedExpense, canEditTextInput, loggedUserId,
            sameUser, specialAccess, showSubCategory, selectedChip, showSelfExpenses, paidList, toPayList,
            imagesCollection
    ...
    //Main view,chips,filters,search
    returnFiltersandChipsView(){
        const {
            expensesList, status, userRole, selectedExpense, canEditTextInput, loggedUserId,
            sameUser, specialAccess, showSubCategory, selectedChip, showSelfExpenses, paidList, toPayList,
            imagesCollection
                    ...
                    <Appbar.Content title="Expenses" titleStyle={[Styles.ffLBlack]}/>
                    {
                        selectedChip === 'DELETED' || this.state.userRole === 30 || (this.state.userRole === 45 && this.state.specialAccess === false)
                            ...
                            <View
                                ...
                                <Fontisto
                                    name={showSelfExpenses ? "toggle-on" : "toggle-off"}
                                ...
                                <Text
                                    style={[Styles.ffLBlack, Styles.f16, Styles.aslCenter, Styles.cWhite]}>Self</Text>
                            </View>
                    }
                </Appbar.Header>
                <View style={{
                                ...
                                return (
                                    <View key={index}>
                                        <TouchableOpacity
                                            ...
                                            activeOpacity={0.7}
                                            style={[Styles.row, Styles.aslCenter, Styles.p5, Styles.m3, {
...

                    {
                        selectedChip === 'PAID' || selectedChip === 'TOPAY'
                            ...
                            paidList.length === 0
                                ...
                                <View style={[Styles.aslCenter]}>
                                    <Text
                                        style={[Styles.ffLBold, Styles.f18, Styles.colorBlue, Styles.alignCenter, Styles.pLeft15]}>No Expenses Found..</Text>
                                </View>
                                :
                            ...
                            expensesList.length === 0
                                ...
                                <View style={[Styles.aslCenter]}>
                                    <Text
                                        style={[Styles.ffLBold, Styles.f18, Styles.colorBlue, Styles.alignCenter, Styles.pLeft15]}>No
                                        Expenses Found..</Text>
                                </View>
                                :
                                <View style={[Styles.mBtm5, Styles.flex1]}>
                                            ...
                                            <View key={index}
                                                ...
                                                <TouchableOpacity
                                                    ...
                                                    <View style={[Styles.row, Styles.jSpaceBet,]}>
                                                        <View>
                                                            <View
                                                                style={[Styles.row, Styles.pTop5, Styles.jSpaceBet,]}>
                                                                <View style={[Styles.row]}>
                                                                    <Text
                                                                        style={[Styles.ffLBlack, Styles.f16, Styles.colorBlue, Styles.pRight5]}>Ticket
                                                                        # </Text>
    ...
    render() {
        ...
        return (
            <View style={[Styles.flex1, Styles.bgDWhite]}>
                ...
                <OfflineNotice/>
                {
                    this.state.createReimbursementModal
                        ?
                        this.returnCreateExpenseDetails()
                        :
                        this.returnFiltersandChipsView()
                }
