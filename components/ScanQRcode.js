...
import {CText, Styles, CSpinner, LoadSVG} from './common'
import Utils from './common/Utils';
import Config from "./common/Config"
import Services from "./common/Services";
import {Appbar, Card, Button,Switch} from "react-native-paper";
    ...
    async requestCameraPermission() {
        try {
            const granted = request(Platform.OS === 'ios' ?
                ...
                if ('granted' === result) {
                        this.setState({QRVisible: true, granted: true})
                    } else {
            ...
            // console.log('camera permss scan###',err);
            Utils.dialogBox('err', '');
            // console.warn(err);
        }
    }
...
//API CALL for ATTENDENCE MARK
    AttendenceMark = () => {
        ...
        self.setState({spinnerBool: true}, () => {
            Services.AuthHTTPRequestForShiftFlow(scanURL, "PUT", body, function (response) {
                if (response.status === 200) {
    ...
    validatingLocation() {
        ...
        if (this.state.longitude && this.state.latitude ) {
            ...
            Alert.alert('', 'Your Location data is missing, Please check your App Location Settings',
                [{
                    text: 'enable', onPress: () => {
                        this.requestLocationPermission();
                    }
    ...
    async requestBackgroundPermission(){
        try {
            const granted = request(Platform.OS === 'ios' ?
                ...
                Alert.alert('App needs Background Location Permissions', 'Select Allow all the time in Settings > Location',
                    [
                        {
                            text: 'Open Settings', onPress: () => {
                                // this.props.navigation.goBack()
    ...
    userAttendenceLog = () => {
        ...
        const body = JSON.stringify({
            ...
            "mocked": self.state.mocked
        });
        // console.log('User Attendence Log BODY', body);
        self.setState({spinnerBool: true}, () => {
            Services.AuthHTTPRequestForShiftFlow(UserAttendenceLogURL, "POST", body, function (response) {
                if (response) {
                    let markedData = response.data;
